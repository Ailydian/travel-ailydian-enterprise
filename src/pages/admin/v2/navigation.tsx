...
const NavigationManager = () => {
  ...
  const handleSave = async (formData: any) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setEditingMenu(null);
        fetchMenus();
        showToast({ type: 'success', title: editingMenu ? 'Menü güncellendi!' : 'Menü oluşturuldu!' });
      } else {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (result.success) {
        fetchMenus();
        showToast({ type: 'info', title: 'Menü silindi' });
      } else {
        showError('Hata', ' + result.error');
      }
    } catch (error) {
      logger.error('Error deleting menu:', error as Error, { component: 'Navigation' });
      showToast({ type: 'error', title: 'Silme başarısız' });
    }
  };
