...
export default function RentalPropertiesManagement() {
  ...
  const deleteProperty = async (id: string) => {
    ...
    try {
      ...
      if (data.success) {
        showToast({ type: 'success', title: 'Mülk başarıyla silindi' });
        fetchProperties();
      } else {
        showToast({ type: 'error', title: data.error || 'Mülk silinemedi' });
      }
    } catch (error) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-6 gap-4 mt-6">
            <div className="bg-gradient-to-br from-purple-500/20 to-purple-500/30 p-4 rounded-lg border border-purple-500/20">
              <p className="text-sm text-purple-400 font-medium">Toplam Mülk</p>
              <p className="text-2xl font-bold text-purple-300 mt-1">{stats.total}</p>
            </div>
            ...
            <div className="bg-gradient-to-br from-blue-500/20 to-blue-500/30 p-4 rounded-lg border border-blue-500/20">
              ...
              <p className="text-2xl font-bold text-blue-300 mt-1">{stats.superhosts}</p>
            </div>
            <div className="bg-gradient-to-br from-rose-500/20 to-rose-500/30 p-4 rounded-lg border border-rose-500/20">
              <p className="text-sm text-rose-400 font-medium">Rezervasyon</p>
              <p className="text-2xl font-bold text-rose-300 mt-1">{stats.totalBookings}</p>
            </div>
            <div className="bg-gradient-to-br from-emerald-500/20 to-emerald-500/30 p-4 rounded-lg border border-emerald-500/20">
              <p className="text-sm text-emerald-400 font-medium">Instant Book</p>
