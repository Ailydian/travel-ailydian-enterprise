...
export default function CommercialVehicleOwnerDashboard() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-amber-50 to-yellow-50">
      {/* Header */}
      <header className="bg-white/10 backdrop-blur-xl border border-b border-white/20/10 sticky top-0 z-40">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400">Minibüs & Kamyonet Dashboard</p>
              </div>
            </div>

            <div className="flex items-center gap-4">
              <Link href="/">
                <button className="flex items-center gap-2 px-4 py-2 text-gray-300 hover:bg-white/5 rounded-lg transition-colors">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        <motion.div
          ...
          <Link href="/commercial-vehicle-owner/vehicles/new">
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="flex items-center gap-2 px-6 py-3 bg-white/10 backdrop-blur-xl border border-white/20 text-orange-600 rounded-xl font-bold shadow-lg hover:shadow-xl transition-all">
        ...
        <motion.div
          ...
          className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl shadow-lg p-6 mb-8">

          <div className="flex items-center justify-between mb-6">
            <h3 className="text-xl font-bold text-white">Son Rezervasyonlar</h3>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/20/10">
                  <th className="text-left py-3 px-4 text-sm font-semibold text-gray-400">Müşteri</th>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-gray-400">Araç</th>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-gray-400">Tarih</th>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-gray-400">Tutar</th>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-gray-400">Durum</th>
                </tr>
              </thead>
              <tbody>
                {recentBookings.map((booking) =>
                <tr key={booking.id} className="border-b border-white/20 hover:bg-white/10 backdrop-blur-xl border border-white/20">
                    <td className="py-4 px-4 text-sm text-white">{booking.customer}</td>
                    <td className="py-4 px-4 text-sm text-gray-400">{booking.vehicle}</td>
                    <td className="py-4 px-4 text-sm text-gray-400">{booking.date}</td>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Link href="/commercial-vehicle-owner/vehicles">
            <motion.div
              whileHover={{ y: -5 }}
              className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all cursor-pointer">

              <Package className="w-12 h-12 text-orange-600 mb-4" />
              <h4 className="text-lg font-bold text-white mb-2">Araçlarım</h4>
              <p className="text-sm text-gray-400">Tüm araçlarınızı görüntüleyin ve yönetin</p>
            </motion.div>
          </Link>
          ...
          <Link href="/commercial-vehicle-owner/analytics">
            <motion.div
              ...
              <p className="text-sm text-gray-400">Gelir ve performans raporları</p>
            </motion.div>
          </Link>
