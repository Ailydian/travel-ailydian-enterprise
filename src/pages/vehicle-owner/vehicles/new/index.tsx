...
import {
  ...
  type Step7FormData,
  type Step8FormData,
} from '@/lib/validation/vehicleSubmissionSchemas';

import Step1VehicleType from '@/app/vehicle-owner/vehicles/new/Step1VehicleType';
import Step2VehicleDetails from '@/app/vehicle-owner/vehicles/new/Step2VehicleDetails';
import Step3Features from '@/app/vehicle-owner/vehicles/new/Step3Features';
import Step4Photos from '@/app/vehicle-owner/vehicles/new/Step4Photos';
import Step5Pricing from '@/app/vehicle-owner/vehicles/new/Step5Pricing';
import Step6Availability from '@/app/vehicle-owner/vehicles/new/Step6Availability';
import Step7Documents from '@/app/vehicle-owner/vehicles/new/Step7Documents';
...
const TOTAL_STEPS = 8;
const LOCAL_STORAGE_KEY = 'vehicle-submission-draft';
...
export default function VehicleSubmissionWizard() {
  const { t } = useTranslation('vehicle-owner');

  const stepTitles = [
    t('vehicleSubmission.step1.title'),
    t('vehicleSubmission.step2.title'),
    t('vehicleSubmission.step3.title'),
    t('vehicleSubmission.step4.title'),
    t('vehicleSubmission.step5.title'),
    t('vehicleSubmission.step6.title'),
    t('vehicleSubmission.step7.title'),
    t('vehicleSubmission.step8.title'),
  ];

  const stepDescriptions = [
    t('vehicleSubmission.step1.subtitle'),
    t('vehicleSubmission.step2.subtitle'),
    t('vehicleSubmission.step3.subtitle'),
    t('vehicleSubmission.step4.subtitle'),
    t('vehicleSubmission.step5.subtitle'),
    t('vehicleSubmission.step6.subtitle'),
    t('vehicleSubmission.step7.subtitle'),
    t('vehicleSubmission.step8.subtitle'),
  ];
      ...
      try {
        ...
        if (savedDraft) {
          ...
          const shouldRestore = window.confirm(
            'Kaydedilmiş bir taslak bulduk. Kaldığınız yerden devam etmek ister misiniz?'
          );
  ...
  const handleFinalSubmit = async (formData: WizardFormData) => {
    ...
    try {
      // Submit to API
      const response = await fetch('/api/vehicles/submit', {
        method: 'POST',
        headers: {
  ...
  const renderStepComponent = () => {
    ...
    switch (currentStep) {
      case 1:
        return <Step1VehicleType {...stepProps} />;
      case 2:
      ...
      case 7:
        return <Step7Documents {...stepProps} />;
      case 8:
                  ...
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span
                      className={`text-xs font-medium ${
                        step === currentStep ? 'text-green-700' : 'text-slate-600'
                      }`}
                    >
                      Adım {step}
                    </span>
          ...
          <form onSubmit={handleSubmit(handleNext)}>
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <button
                ...
                className="px-8 py-3 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-lg font-semibold hover:from-green-700 hover:to-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center gap-2 shadow-lg"
              >
                {isSubmitting ? (
                  <>
