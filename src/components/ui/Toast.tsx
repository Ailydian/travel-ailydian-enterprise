/**
 * Toast/Alert Component - Production-Grade Notifications
 * Features: Auto-dismiss, positioning, animations, accessibility
 */
...
const ToastContainer: React.FC<{ toasts: Toast[]; removeToast: (id: string) => void }> = ({ toasts, removeToast }) => {
  ...
  return createPortal(
    <div className="fixed top-4 right-4 z-50 flex flex-col gap-3 max-w-md" role="region" aria-live="polite">
      {toasts.map((toast) => (
...
const ToastItem: React.FC<{ toast: Toast; onClose: () => void }> = ({ toast, onClose }) => {
  ...
  return (
    <div className={`flex items-start gap-3 p-4 rounded-lg border shadow-lg animate-slideIn ${styles[toast.type]}`} role="alert">
      <div className="flex-shrink-0 mt-0.5">{toast.icon || icons[toast.type]}</div>
