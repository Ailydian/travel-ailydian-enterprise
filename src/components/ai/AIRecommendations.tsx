...
export const AIRecommendations: React.FC<AIRecommendationsProps> = ({
    ...
    return (
      <motion.div
        ...
        <button
          ...
          <div className="absolute -top-2 -right-2 bg-lydian-error text-white text-xs font-bold w-6 h-6 rounded-full flex items-center justify-center animate-bounce">
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, x: 50 }}
      className={`fixed top-20 right-4 w-96 max-h-[calc(100vh-6rem)] bg-gradient-to-br from-slate-900 via-black to-slate-800 rounded-2xl shadow-2xl border border-purple-100 z-40 overflow-hidden hidden lg:block ${className}`}>
      ...
      <div className="bg-gradient-to-r from-purple-600 to-pink-600 p-4 text-white">
        ...
        <AnimatePresence>
          ...
          <motion.div
            ...
            className="mt-2 text-xs text-purple-100 bg-white/10 backdrop-blur-xl border border-white/20 rounded-lg p-2">
      ...
      <div className="max-h-96 overflow-y-auto p-4 space-y-3">
        ...
        <>
          ...
          <div key={i} className="animate-pulse flex gap-3">
                <div className="w-20 h-20 bg-white/10 backdrop-blur-xl border border-white/20 rounded-lg" />
                <div className="flex-1 space-y-2">
                  <div className="h-4 bg-white/10 backdrop-blur-xl border border-white/20 rounded w-3/4" />
                  <div className="h-3 bg-white/10 backdrop-blur-xl border border-white/20 rounded w-1/2" />
        ...
        <motion.div
          ...
          transition={{ delay: index * 0.1 }}>
              ...
              <Link href={`/tours/${typeof tour.title === 'string' ? tour.title.toLowerCase().replace(/[^a-z0-9ğüşıöçĞÜŞİÖÇ\s]/g, '').replace(/\s+/g, '-') : '#'}`}>
                <div className="group flex gap-3 p-3 hover:bg-purple-50 rounded-xl transition-all cursor-pointer border border-white/20 hover:border-purple-200">
                  {/* Image */}
                  ...
                  {/* Arrow */}
                  <ChevronRight className="w-5 h-5 text-gray-300 group-hover:text-purple-600 group-hover:translate-x-1 transition-all flex-shrink-0 self-center" />
                </div>
              </Link>
            </motion.div>
        )
