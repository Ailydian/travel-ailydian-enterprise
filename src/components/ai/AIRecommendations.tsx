...
export const AIRecommendations: React.FC<AIRecommendationsProps> = ({
    ...
    return (
      <motion.div
        ...
        <button
          ...
          className="group relative bg-gradient-to-r from-lydian-secondary to-pink-600 text-lydian-text-inverse p-4 rounded-full shadow-2xl hover:shadow-purple-500/50 transition-all">

          <Sparkles className="w-6 h-6 animate-pulse" />
          <div className="absolute -top-2 -right-2 bg-lydian-error text-lydian-text-inverse text-xs font-bold w-6 h-6 rounded-full flex items-center justify-center animate-bounce">
          ...
          <div className="absolute right-full mr-4 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
            <div className="bg-gray-900 text-lydian-text-inverse text-sm px-3 py-2 rounded-lg whitespace-nowrap shadow-xl">
              AI Önerilerim
            </div>
          </div>
        </button>
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, x: 50 }}
      className={`fixed top-20 right-4 w-96 max-h-[calc(100vh-6rem)] bg-lydian-glass-dark rounded-2xl shadow-2xl border border-purple-100 z-40 overflow-hidden hidden lg:block ${className}`}>

      {/* Header */}
      <div className="bg-gradient-to-r from-lydian-secondary to-pink-600 p-4 text-lydian-text-inverse">
        ...
        <button
          ...
          className="text-xs text-purple-100 hover:text-lydian-text-inverse flex items-center gap-1 transition-colors">
        ...
        <AnimatePresence>
          ...
          <motion.div
            ...
            className="mt-2 text-xs text-purple-100 bg-lydian-glass-dark-medium rounded-lg p-2">
      ...
      <div className="max-h-96 overflow-y-auto p-4 space-y-3">
        ...
        <>
          ...
          <div key={i} className="animate-pulse flex gap-3">
                <div className="w-20 h-20 bg-lydian-bg-active rounded-lg" />
                <div className="flex-1 space-y-2">
                  <div className="h-4 bg-lydian-bg-active rounded w-3/4" />
                  <div className="h-3 bg-lydian-bg-active rounded w-1/2" />
        ...
        <motion.div
          ...
          transition={{ delay: index * 0.1 }}>
              ...
              <Link href={`/tours/${typeof tour.title === 'string' ? tour.title.toLowerCase().replace(/[^a-z0-9ğüşıöçĞÜŞİÖÇ\s]/g, '').replace(/\s+/g, '-') : '#'}`}>
                <div className="group flex gap-3 p-3 hover:bg-purple-50 rounded-xl transition-all cursor-pointer border border-lydian-border hover:border-purple-200">
                  {/* Image */}
                  ...
                  className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" />
                    ...
                    {/* Match Score Badge */}
                    <div className="absolute top-1 right-1 bg-gradient-to-r from-lydian-secondary to-pink-600 text-lydian-text-inverse text-xs font-bold px-1.5 py-0.5 rounded">
                      %{tour.matchScore}
                    </div>
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="font-semibold text-sm text-lydian-text-inverse line-clamp-1 group-hover:text-purple-600 transition-colors">
                      {tour.title}
                    </h4>
                    <div className="flex items-center gap-1 text-xs text-lydian-text-dim mt-1">
                  ...
                  {/* Arrow */}
                  <ChevronRight className="w-5 h-5 text-lydian-text-muted group-hover:text-purple-600 group-hover:translate-x-1 transition-all flex-shrink-0 self-center" />
                </div>
              </Link>
            </motion.div>
        )
