...
export const AIAnswerBlock: React.FC<AIAnswerBlockProps> = ({
  ...
  return (
    <article
      className="ai-answer-block bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-8 my-8"
      itemScope
    ...
    >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {trustSignals.map((signal, index) => (
              ...
              >
                ...
                <div className="text-sm text-gray-300 mb-1">{signal.label}</div>
        ...
        >
          ...
          <div className="space-y-4">
            {faqItems.map((faq, index) => (
              <details
                key={index}
                className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-lg p-4 group"
                itemScope
                itemProp="mainEntity"
      ...
      {citations.length > 0 && (
        <footer className="mt-6 pt-6 border-t border-white/10">
          <h4 className="text-sm font-semibold text-gray-300 mb-3">Kaynaklar:</h4>
