  ...
  return (
    <div className="space-y-6">
      <div className="text-center space-y-2">
        <h2 className="text-3xl sm:text-4xl font-black text-lydian-text-inverse">{t('explore.collections.title')}</h2>
        <p className="text-lydian-text-dim text-lg">{t('explore.collections.subtitle')}</p>
      </div>
                ...
                <div className="group relative h-72 rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all">
                  ...
                  <div className="absolute inset-0 p-6 flex flex-col justify-between">
                    <div className="flex justify-between items-start">
                      <div className="w-12 h-12 bg-lydian-glass-dark-heavy backdrop-blur-md rounded-xl flex items-center justify-center">
                        <Icon className="w-6 h-6 text-lydian-text-inverse" />
                      </div>
                      <span className="px-3 py-1 bg-lydian-glass-dark-heavy backdrop-blur-md rounded-full text-xs font-semibold text-lydian-text-inverse">
                        {collection.itemIds.length} {t('explore.items')}
                      </span>
                    </div>
                    <div>
                      <h3 className="text-2xl font-bold text-lydian-text-inverse mb-2">{collectionData.title}</h3>
                      <p className="text-lydian-text-inverse/90 text-sm mb-4">{collectionData.tagline}</p>
                      <div className="flex items-center gap-2 text-lydian-text-inverse font-semibold">
