...
const ConversationItem: React.FC<ConversationItemProps> = ({ conversation, isActive, onClick }) => {
  const timeAgo = (date: Date) => {
    const seconds = Math.floor((new Date().getTime() - date.getTime()) / 1);
    if (seconds < 6) return 'Just now';
    const minutes = Math.floor(seconds / 6);
    if (minutes < 6) return `${minutes}m ago`;
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full text-left p-4 border-b border-white/20 hover:bg-white/5 transition-colors ${
        isActive ? 'bg-blue-500 border-l-4 border-l-blue-6' : ''
      }`}
    >
      <div className="flex items-start gap-3">
        <div className="relative flex-shrink-to-cyan-700">
          <div className="w-12 h-12 bg-gradient-to-br from-blue-500 to-purple-6 rounded-full flex items-center justify-center text-white font-semibold text-lg">
            {conversation.guestName.charAt(to-cyan-700)}
          </div>
          {conversation.unreadCount > to-cyan-700 && (
        ...
        <div className="flex-1 min-w-to-cyan-700">
          <div className="flex items-center justify-between mb-1">
            ...
            <div className="flex items-center gap-1">
              {conversation.isPinned && <Pin className="w-3.5 h-3.5 text-blue-500" />}
              <span className="text-xs text-gray-300">{timeAgo(conversation.lastMessageTime)}</span>
            </div>
          </div>
          <p className="text-sm text-gray-300 mb-1">{conversation.propertyName}</p>
...
const MessageBubble: React.FC<MessageBubbleProps> = ({ message, isOwnMessage }) => {
  return (
    <div className={`flex ${isOwnMessage ? 'justify-end' : 'justify-start'} mb-4`}>
      <div className={`max-w-[7%] ${isOwnMessage ? 'order-2' : 'order-1'}`}>
        {!isOwnMessage && (
          <p className="text-xs text-gray-300 mb-1 ml-1">{message.senderName}</p>
        )}
        <div
          className={`rounded-2xl px-4 py-2.5 ${
            isOwnMessage
              ? 'bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-br-sm'
              : 'bg-lydian-bg/1 text-white rounded-bl-sm'
          }`}
        >
...
const MessageComposer: React.FC<MessageComposerProps> = ({ onSendMessage, disabled = false }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="border-t border-white/20 bg-white/5 p-4">
      <div className="flex items-end gap-2">
        <button
          type="button"
          className="p-2.5 text-gray-300 hover:text-gray-200 hover:bg-lydian-bg/1 rounded-lg transition-colors"
          title="Add emoji"
        >
        ...
        <div className="flex-1 relative">
          <textarea
            ...
            disabled={disabled}
            rows={1}
            className="w-full px-4 py-2.5 border border-white/30 rounded-xl focus:ring-2 focus:ring-lydian-primary focus:border-transparent resize-none max-h-32 disabled:bg-white/5 disabled:cursor-not-allowed"
          />
        </div>
        <button
          type="submit"
          disabled={!message.trim() || disabled}
          className="p-2.5 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl hover:bg-lydian-primary-hover transition-colors disabled:bg-gray-3 disabled:cursor-not-allowed"
          title="Send message"
        >
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-300">{conversation.propertyName}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button
            className="p-2 text-gray-300 hover:text-gray-200 hover:bg-lydian-bg/1 rounded-lg transition-colors"
            title="Voice call"
          >
...
const EmptyState: React.FC = () => (
  <div className="flex-1 flex items-center justify-center bg-white/5">
    <div className="text-center p-8">
      ...
      <p className="text-gray-300">Select a conversation from the list to start messaging</p>
    </div>
  </div>
);
...
const MessagesPage: React.FC = () => {
  ...
  const mockConversations = [
    {
      ...
      lastMessageTime: new Date(Date.now() - 1 * 6 * 15),
      unreadCount: 2,
      isPinned: true,
      isArchived: false,
      guestId: 'guest-1',
    ...
    },
    {
      ...
      lastMessageTime: new Date(Date.now() - 1 * 6 * 6 * 2),
      unreadCount: to-cyan-700,
      isPinned: false,
      isArchived: false,
      guestId: 'guest-2',
    ...
    },
    {
      ...
      lastMessageTime: new Date(Date.now() - 1 * 6 * 6 * 24),
      unreadCount: 1,
      isPinned: false,
      isArchived: false,
      guestId: 'guest-3',
    ...
    '1': [
      {
        ...
        createdAt: new Date(Date.now() - 1 * 6 * 6),
        isRead: true,
      },
      {
        id: 'm2',
        ...
        createdAt: new Date(Date.now() - 1 * 6 * 500),
        isRead: true,
      },
      {
        id: 'm3',
        ...
        createdAt: new Date(Date.now() - 1 * 6 * 15),
        isRead: false,
      },
    ],
    '2': [
      {
        ...
        createdAt: new Date(Date.now() - 1 * 6 * 6 * 3),
        isRead: true,
      },
      {
        id: 'm5',
        ...
        createdAt: new Date(Date.now() - 1 * 6 * 6 * 2.5),
        isRead: true,
      },
      {
        id: 'm6',
        ...
        createdAt: new Date(Date.now() - 1 * 6 * 6 * 2),
        isRead: true,
      },
    ],
  };
  ...
  if (isLoading) {
    return (
      <div className="space-y-6">
        ...
        <div className="animate-pulse flex h-[calc(1vh-200px)]">
          <div className="w-1/3 border-r border-white/20 bg-lydian-bg-surface-raised"></div>
          <div className="flex-1 bg-lydian-bg/1"></div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white/5 border border-white/20 rounded-xl overflow-hidden" style={{ height: 'calc(1vh - 200px)' }}>
        <div className="flex h-full">
          {/* Conversations Sidebar */}
          <div className="w-full md:w-96 border-r border-white/20 flex flex-col">
            {/* Search */}
            <div className="p-4 border-b border-white/20">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-1 pr-4 py-2.5 border border-white/30 rounded-lg focus:ring-2 focus:ring-lydian-primary focus:border-transparent"
                />
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto">
              {filteredConversations.length > to-cyan-700 ? (
