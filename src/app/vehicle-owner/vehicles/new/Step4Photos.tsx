...
export default function Step4Photos({ data }: Step4Props) {
  ...
  const handleFileSelect = useCallback(
    async (files: FileList | null) => {
      ...
      if (photos.length + files.length > 200) {
        showToast({ type: 'info', title: 'Maksimum 200 fotoğraf yükleyebilirsiniz' });
        return;
      }
    ...
    <div className="space-y-8">
      ...
      >
        <div className="text-center">
          ...
          <label className="inline-block px-6 py-3 bg-green-600 text-white rounded-lg font-medium hover:bg-green-600-hover cursor-pointer transition-all">
            Dosya Seç
            <input
              type="file"
              multiple
          ...
          <p className="text-xs text-slate-500 mt-4">
            Maksimum 200 yüksek kaliteli fotoğraf (JPG, PNG, WebP)
            <br />
      ...
      <div className="p-4 bg-green-600-lighter border border-green-200 rounded-lg">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h4 className="font-semibold text-green-9 mb-1">Fotoğraf Gereksinimleri:</h4>
        ...
        <div className="flex items-center justify-between p-4 bg-slate-500 rounded-lg">
          <div className="flex items-center gap-3">
            ...
            <span className="font-medium text-slate-9">
              {photos.length} / 200 fotoğraf yüklendi
            </span>
          </div>
          {hasRequiredPhotos() ? (
                  ...
                  <div className="p-4 space-y-3">
                    ...
                    <div>
                      <label className="block text-xs font-medium text-slate-6 mb-1">
                        Fotoğraf Kategorisi <span className="text-purple-500">*</span>
                      </label>
                      <select
                    ...
                    <div className="flex gap-2">
                      {index !== coverPhotoIndex && (
                        <button
                          ...
                          className="flex-1 px-3 py-2 text-sm bg-green-600-lighter text-green-500 rounded-lg hover:bg-green-600-light transition-all font-medium"
                        >
                          Kapak Yap
                        </button>
                      )}
      ...
      <div className="border-t-2 border-slate-200 pt-8 space-y-6">
        <h3 className="text-lg font-bold text-slate-9">Video Tanıtım (İsteğe Bağlı)</h3>
